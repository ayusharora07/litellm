...
@pytest.mark.asyncio
@pytest.mark.parametrize("asynchronous", [False, True], ids=["sync", "async"])
async def test_native_callback_task_and_context_boundaries(ocr_server: RecordingServer, asynchronous: bool) -> None:
    from contextvars import ContextVar
...
@pytest.mark.asyncio
@pytest.mark.parametrize("phase", ["token", "pre_call"])
async def test_native_callback_cancelled_error_aborts_before_provider(ocr_server: RecordingServer, phase: str) -> None:
    from litellm.rust_bridge import _native

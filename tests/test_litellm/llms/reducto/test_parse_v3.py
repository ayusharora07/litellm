...
@pytest.mark.asyncio
async def test_unknown_model_uses_current_protocol_without_local_rejection(disable_aiohttp_transport, respx_mock):
    parse_route = respx_mock.post("https://platform.reducto.ai/parse").respond(json=_reducto_parse_response())
    ...
    assert parse_route.called
    assert json.loads(parse_route.calls[0].request.read()) == {"input": "reducto://already-uploaded.pdf"}
